...
#   gmt grdpaste, from GMT6
#   gmt grdcut, from GMT6
#
# Notice:
...
$SRTM_DIR="/public/data-nfs/TopographyModels/SRTMv4.1/tiles/GeoTIFF";
...
Notice:
  1) This script is suitable for GMT 6.
...
if(($xmax-$xmin)<=5 && ($ymax-$ymin)<=5){
  ...
  system("gmt grdcut $srtm_topo -G$grd_topo -R$xmin/$xmax/$ymin/$ymax");
  system("rm $srtm_topo");
}else{
...
sub TransSRTM {
  ...
  system("gmt grdpaste tmp_srtm1.grd tmp_srtm2.grd -G$outfile -fg");
...
sub ReadSRTM {
  ...
  if($file_lu ne $file_ld){
    ...
    if($file_rd ne $file_ld){
      ...
      system("gmt grdpaste tmp_left.grd tmp_right.grd -G$outfile -fg");
